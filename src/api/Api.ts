...
const instance = axios.create({
  ...
  baseURL:
    globalParameters.servUrl +
    '/ords/r/gdebus_pck/' +
    globalParameters.clientVersion +
    '/get_data',
  headers: {
...
export const paramsAPI = {
  async callServerFunction<T extends TGetFuncObjParameters>(
    data: T,
    srvUrl?: string,
  ): Promise<any> {
    try {
      ...
      try {
        ...
        respInst = await instance.post<AxiosResponse>(srvUrl || '', formData, {
          headers: {
  ...
  async getFuncObjParam<K extends TFuncParameter[]>(
    funcName: string,
    funcParametersArr: K,
  ): Promise<TGetFuncObjParameters> {
  ...
  async getFuncParamFromServer(
    funcName: string,
    srvUrl?: string,
  ): Promise<TGetFuncObjParameters> {
    let arrParam = [
      {name: 'cFUNC_NAME', value: funcName, data_type: 'VARCHAR2'},
    ];
    try {
      let respObjParam = await this.getFuncObjParam<TFuncParameter[]>(
        'GET_FUNCTIONS_PARAMS_JSON',
        arrParam,
      );
      let respServFunc = await this.callServerFunction<TGetFuncObjParameters>(
        respObjParam,
        srvUrl,
      );
  ...
  async getFuncParamFromObject(
    funcName: string,
  ): Promise<TGetFuncObjParameters | null> {
    try {
      if (globalParameters.funcParameters.length === 0) {
        return null;
      }
      let filteredParams = _.filter(
        globalParameters.funcParameters,
        (t: TFuncParameters) => t.object_name == funcName,
      );
  ...
  async getFuncParamObjWithoutValues(
    funcName: string,
    srvUrl?: string,
  ): Promise<TGetFuncObjParameters> {
  ...
  async addValuesIntoParamObj<T extends TGetFuncObjParameters>(
    objParams: T,
    othersParamsVals?: any,
  ): Promise<T> {
    if (_.isEmpty(objParams.data.parameters) && _.isEmpty(othersParamsVals)) {
      return objParams;
    }
    //Берем значения параметров либо из хранилища либо из clickedAppRoute
    let paramsArr = objParams.data.parameters;
    let parametersChecking: Array<boolean> = _.map(paramsArr, () => {
      return false;
    });
    for (const [index, params] of paramsArr.entries()) {
      ...
      try {
        otherParam =
          othersParamsVals[paramName] === 0
            ? 0
            : othersParamsVals[paramName] || null;
      } catch (error) {
        otherParam = null;
      }
...
type TPostAPI = {
  getStopsJson: (
    ...
    curLatitude?: number,
    curLongitude?: number,
  ) => Promise<TStopsSuccessResponse | TErrorResponse>;
  getForecastJson: (
    stopId: string,
    srvUrl?: string,
  ) => Promise<TForecastSuccessResponse | TErrorResponse>;
  getCities: (
    skip: number,
    limit: number,
    searchText?: string,
    cityId?: string,
  ) => Promise<TCitiesSuccessResponse | TErrorResponse>;
};
...
export const postAPI: TPostAPI = {
  async getStopsJson(
    ...
    try {
      const data = {
        NACCOUNT_ID: accountId,
        NFIRST_ROWNUM: skip,
        NLAST_ROWNUM: limit,
        CFILTER: searchText,
        NID: stopId,
        NCITY_ID: cityId,
        NCUR_LATITUDE: curLatitude,
        NCUR_LONGITUDE: curLongitude,
      };
      const respParams = await paramsAPI.getFuncParamObjWithoutValues(
        'GET_STOPS_JSON',
        srvUrl,
      );
      const respParamsVal = await paramsAPI.addValuesIntoParamObj(
        respParams,
        data,
      );
      const restData = await paramsAPI.callServerFunction(
        respParamsVal,
        srvUrl,
      );
      return restData;
